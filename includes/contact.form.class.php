<?php
	...
	class contactForm extends formValidation{
		private $name,$email,$message,$mails,$insert,$conn;
		...
		public function check_db_email($email){
			$count=0;
			$statement = $this->conn->prepare("SELECT * FROM contact WHERE email=:parameter");
		...
		public function validate(){
			if(parent::validate_name($this->name)){
				if(parent::validate_message($this->message)){
